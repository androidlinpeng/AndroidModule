...
package com.apicloud.code.zxing.utils;

import android.os.Bundle;
import android.os.Handler;
import android.os.Message;

import com.apicloud.R;
import com.apicloud.code.zxing.ScanManager;
import com.apicloud.code.zxing.camera.CameraManager;
...
public class CaptureActivityHandler extends Handler {
	...
	public void handleMessage(Message message) {
		...
		int id = message.what;
		int restart_previewId = R.id.restart_preview;
		int decode_succeededId =R.id.decode_succeeded;
		int decode_failedId = R.id.decode_failed;
		int decode_errorId = R.id.decode_error;
		int return_scan_resultId =R.id.return_scan_result;
		if (id == restart_previewId){
			restartPreviewAndDecode();
		}else if (id == decode_succeededId){
			state = State.SUCCESS;
			Bundle bundle = message.getData();
			scanManager.handleDecode((Result) message.obj, bundle);
		}else if (id == decode_failedId){
			state = State.PREVIEW;
			cameraManager.requestPreviewFrame(decodeThread.getHandler(), R.id.decode);
		}else if (id == decode_errorId){
			scanManager.handleDecodeError((Exception)message.obj);
		}else if (id == return_scan_resultId){
